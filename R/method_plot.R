...
plot.glmdisc <- function(x) {
  ...
  if (interactive()) graphics::par(ask = TRUE)
  ...
  if (sum(x@parameters$types_data == "numeric") > 0) {
    modele_gam <- gam::gam(stats::as.formula(paste("labels ~", paste("s(", gsub("+", ",5) + s(", paste0(colnames(x@disc.data)[which(x@parameters$types_data == "numeric"), drop = FALSE], collapse = "+"), fixed = TRUE), ",5)"))),
      data = x@cont.data,
      family = "binomial"
    )

    for (j in which(x@parameters$types_data == "numeric")) {
      gam::plot.Gam(modele_gam,
        ask = FALSE,
        terms = paste0("s(", colnames(x@disc.data)[j], ", 5)"),
        se = TRUE,
        main = paste("Plot of a GAM fit to attribute", colnames(x@disc.data)[j], "and resulting discretization.")
      )
      graphics::par(new = TRUE)
      plot(x = x@cont.data[, j], y = x@disc.data[, j], type = "p", axes = FALSE, bty = "n", xlab = "", ylab = "")
      graphics::axis(side = 4, at = pretty(as.numeric(x@disc.data[, j])))
