...
test_that("discretize_link works with one-dimensional continuous data", {
  ...
  link <- list(nnet::multinom(e ~ x, data = data.frame(e = factor(xd), x = x), maxit = 50))
...
test_that("discretize_link works with multi-dimensional continuous data", {
  ...
  link <- list()
  for (j in 1:3) {
    link[[j]] <- nnet::multinom(e ~ x, data = data.frame(e = factor(xd[, j]), x = x[, j]), maxit = 50)
  }
  res <- discretize_link(link, data.frame(x), 3)
  expect_type(res, "character")
  expect_length(res, 300)
  for (j in 1:3) {
