...
sem_polytomique <- function(predictors,labels,validation=TRUE,test=TRUE,criterion='gini',iter=1000,m_depart=20,reg_type='poly') {
     ...
     if (criterion %in% c('gini','aic','bic')) {
          if (length(labels)==length(predictors[,1])) {
               ...
               types_data <- apply(predictors,2,class)
               ...
               # if (interact==TRUE) {
               #      if (!methods::requireNamespace("Rmixmod", quietly = TRUE)) {
               #           stop("Rmixmod not installed, cannot process interactions.")
               #      } else {
               #           melange <- Rmixmod::mixmodCluster(data.frame(predictors),nbCluster=2,
               #                       models = NULL, criterion = "BIC",
               #                       weight = NULL)
               #
               #           # Implémentation naïve
               #
               #           p_x <- mixmodPredict(data.frame(predictors),melange@bestResult)
               #
               #           p_delta_x <- matrix(0, c(n,d,d))
               #
               #           for (i in 1:n) {
               #                for (k in 1:d) {
               #                     for (l in (k+1):d) {
               #                          p_delta_x[i,k,l] <- abs(p_x[])
               #                     }
               #                }
               #           }
               #
               ...
               if (test==TRUE) {
                    return(methods::new(Class = "discretization", parameters = list(test,validation,criterion,iter,m_depart,reg_type), best.disc = best.disc, performance = list(performance,criterion_iter), disc.data = data.frame(cbind(discretize_link(best.disc[[2]],predictors[ensemble[[3]],])),labels = labels[ensemble[[3]]]), cont.data = data.frame(cbind(predictors[ensemble[[3]],]),labels = labels[ensemble[[3]]])))
